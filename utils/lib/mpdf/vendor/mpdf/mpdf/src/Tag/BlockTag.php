...
abstract class BlockTag extends Tag
{
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		$lastbottommargin = 0;
		if (
			$this->mpdf->blockjustfinished && !count($this->mpdf->textbuffer)
		...
		if (
			isset($properties['PAGE-BREAK-INSIDE']) && strtoupper($properties['PAGE-BREAK-INSIDE']) === 'AVOID'
		...
		if (isset($properties['FLOAT']) && strtoupper($properties['FLOAT']) === 'RIGHT' && !$this->mpdf->ColActive) {
			...
			if (($setwidth + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) > $maxw
				...
				if (
					$l_max < $r_max && ($setwidth + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) <= ($container_w - $r_width)
					&& (($container_w - $r_width) - ($currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr)) > $doubleCharWidth
				) {
					$this->mpdf->ClearFloats('LEFT', $this->mpdf->blklvl - 1);
				} elseif (
					$r_max < $l_max && ($setwidth + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) <= ($container_w - $l_width)
			...
			if (($setwidth + $currblk['margin_left'] + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) > $maxw
				...
				// Too narrow to fit - try to move down past L or R float
				if (
					$l_max < $r_max && ($setwidth + $currblk['margin_left'] + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) <= ($container_w - $r_width)
					&& (($container_w - $r_width) - ($currblk['margin_right'] + $currblk['margin_left'] + $bdl + $pdl + $bdr + $pdr)) > $doubleCharWidth
				) {
					$this->mpdf->ClearFloats('LEFT', $this->mpdf->blklvl - 1);
				} elseif (
					$r_max < $l_max && ($setwidth + $currblk['margin_left'] + $currblk['margin_right'] + $bdl + $pdl + $bdr + $pdr) <= ($container_w - $l_width)
		...
		if (isset($currblk['css_set_width'])) {
			if (
				isset($properties['MARGIN-LEFT'], $properties['MARGIN-RIGHT'])
		...
		if ($tag === 'OL' || $tag === 'UL') {
			...
			if (
				strtolower($this->mpdf->list_auto_mode) === 'mpdf' && isset($currblk['list_style_position'])
				...
				if (
					isset($properties['PADDING-RIGHT']) && $properties['PADDING-RIGHT'] === 'auto'
	...
	public function close(&$ahtml, &$ihtml)
	{
		...
		if (isset($this->mpdf->blk[$this->mpdf->blklvl]['float_endpos']) && $this->mpdf->blk[$this->mpdf->blklvl]['float_endpos'] > $currpos) {
			...
			$mod_result = fmod($float_endpos * 1000, 1000000);  // Utiliza fmod para operar con floats
			$this->mpdf->y = $mod_result / 1000;
		}
		/* -- END CSS-FLOAT -- */

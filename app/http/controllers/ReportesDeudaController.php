...
class ReportesDeudaController extends Controller
{
  ...
  {    
    ...
    switch($tipo){
      ...
      break;
      case '2': # por vendedor
        $nombreVendedor = isset($vendedores[0]['nombres']) ? $vendedores[0]['nombres'] : '';
        $titulo = "Cobranza de Vendedor " . strtoupper($nombreVendedor);
      break;
      case '3': # por ruta
  ...
  public function reporteCobros($id_vendedor){
    ...
    if ($fecha_inicio && $fecha_fin) {
        $whereFecha = "AND DATE(co.fecha) BETWEEN '$fecha_inicio' AND '$fecha_fin'";
        $titulo_fecha = 'Desde el día '.$fecha_inicio.' hasta '.$fecha_fin;
      } elseif ($fecha_inicio) {
        $whereFecha = "AND DATE(co.fecha) >= '$fecha_inicio'";
        $titulo_fecha = 'Desde el día '.$fecha_inicio;
      } elseif ($fecha_fin) {
        $whereFecha = "AND DATE(co.fecha) <= '$fecha_fin'";
  ...
  public function hojaVisitaRuta($id_vendedor = null)
  {
    ...
    foreach ($clientes as $cliente) {
      ...
      if ($mercado !== $mercadoActual) {
        ...
        $rowTable .= "
          <tr>
            <td colspan='4' style='text-align:left;background:#e9e9e9;font-weight:bold;padding:4px 6px;'>MERCADO: {$mercado}</td>
          </tr>
        ";
      }
      $n++;
      ...
      $rowTable .= "
        <tr>
          ...
          <td style='text-align:left;'>{$nombre}<br><span style='font-size:9px;color:#555;'>{$documento}</span></td>
    ...
    if ($rowTable === '') {
      $rowTable = "<tr><td colspan='4' style='text-align:center;padding:12px;'>No se encontraron clientes para el filtro seleccionado.</td></tr>";
    }
            ...
            <table border='1' style='width: 100%; text-align: center; border-collapse: collapse; font-size:11px;'>
                <thead>
                  <tr style='background:#d0d0d0;'>
                      <th style='padding: 3px 6px; width:6%;'>N°</th>
                      <th style='padding: 3px 6px; width:42%;'>CLIENTE</th>
                      <th style='padding: 3px 6px; width:35%;'>DIRECCIÓN</th>
                      <th style='padding: 3px 6px; width:17%;'>TELÉFONO</th>
                  </tr>
                </thead>
                <tbody>
                  $rowTable
  ...
  public function reporteVentaPorProducto2()
  {
    ...
    foreach ($listaVentas as $key => $venta) {
      ...
      $productos = ($venta['productos'] == 0) ? 1 : $venta['productos'];
      $precio_sin_igv = ($venta['precio']/$productos) / 1.18;
      $precio_unitario = number_format($precio_sin_igv, 2);
      $costo_sin_igv = ($venta['costo']/$productos) / 1.18;
  ...
  public function reporteVentaPorProducto()
  {
    ...
    foreach ($listaVentas as $key => $venta) {
      // Precio y costo CON IGV, tal como se muestran en el almacen (productos.precio/costo)
      $productos = ($venta['productos'] == 0) ? 1 : $venta['productos'];
      $precio_unitario = number_format($venta['precio']/$productos, 2);
      $costo_unitario = number_format($venta['costo']/$productos, 2);
      $utilidad_unitario = $precio_unitario-$costo_unitario;
      $subtotal_utilidad = $utilidad_unitario * $venta['cantidad'];
      $subtotal = $precio_unitario*$venta['cantidad'];
  ...
  public function reporteVentasVendedor(){
    ...
    foreach ($listaVentas as $key => $venta) {
      ...
      // Los precios en la BD incluyen IGV (18%), debemos quitarlo
      $productos = ($venta['productos'] == 0) ? 1 : $venta['productos'];
      $precio_con_igv = $venta['precio']/$productos;
      $precio_sin_igv = $precio_con_igv / 1.18; // Quitar el 18% de IGV
      $precio_unitario = number_format($precio_sin_igv, 2);

      $costo_unitario = $venta['costo']/$productos;
      $subtotal = $precio_unitario*$venta['cantidad'];
      $total_venta += $subtotal;

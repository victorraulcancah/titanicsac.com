     ...
     public function cotizacionesCuotas($coti)
    {
        try {
            ...
            if ($result_cotizacion->num_rows > 0) {
                ...
                // Obtener el vendedor de la cotizaci車n
        ...
        ";
                ...
                <body>
                    ...
                    <div class='content'>
                        <ul>
                            ...
                            <li><strong>N迆mero:</strong> {$cotizacion['numero']}</li>
                    ...
                    while ($cuota = $result_cuotas->fetch_assoc()) {
                        // Mostrar el usuario que cobr車, o "Sin pagar" si no hay usuario
                        $usuario_mostrar = !empty($cuota['usuario_cobro']) ? $cuota['usuario_cobro'] : 'Sin pagar';
                        $fechaDate = ($cuota['fecha'] == '0000-00-00') ? '-----' : $cuota['fecha'];
                        $totalpagado = $cuota['monto'] + $totalpagado;
                        $tipoPago = ($cuota['tipo_pago'] == NULL) ? '-----' : $cuota['tipo_pago'];
                        $html .= "
                    <tr>
                        <td>{$contador}</td>
                    ...
                    <div class='footer'>
                        <p> TOTAL PAGADO : S/ " . number_format($totalpagado, 2) . "</p>
                    </div>
                </body>
                </html>";
                    $mpdf = new \Mpdf\Mpdf();
                ...
                echo "<p>No se encontr車 la cotizaci車n con el ID: $idCoti</p>";
            }
        } catch (Exception $e) {

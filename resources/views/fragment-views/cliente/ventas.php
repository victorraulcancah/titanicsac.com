            ...
            <div class="card-body"  >
                <div class="table-responsive">
                    ...
                    <div class="card-title-desc">
                        <a       style="margin-top: 10px;" href="/ventas/productos" class="btn btn-primary button-link"><i class="fa fa-plus"></i> Facturar Productos</a>
                        <a       style="margin-top: 10px;" hidden href="/ventas/servicios" class="btn btn-primary button-link"><i class="fa fa-plus"></i> Facturar Servicios</a>
                        <a       style="margin-top: 10px;" href="/nota/electronica" class="btn btn-success button-link"><i class="fa fa-plus"></i> Agregar Nota Electronica</a>
                        <button  style="margin-top: 10px;" data-bs-toggle="modal" data-bs-target="#ventas-pdf-reporte" class="btn btn-info"><i class="fa fa-file-pdf-o"></i> Exportar a PDF Reporte de Venta</button>
                        <button  style="margin-top: 10px;" data-bs-toggle="modal" data-bs-target="#ventas-pdf-reporteganancia" class="btn btn-info"><i class="fa fa-file-pdf-o"></i>Reporte de Venta Ganancias</button>
                        <button  style="margin-top: 10px;" data-bs-toggle="modal" data-bs-target="#ventas-text-reporte" class="btn btn-info"><i class="fa fa-file-text"></i> Exportar a TXT</button>
                        <button  style="margin-top: 10px;" data-bs-toggle="modal" data-bs-target="#ventas-xls-reporte" class="btn btn-info"><i class="fa fa-file-text"></i> Exportar a al formato "xls"</button>
                        <button  style="margin-top: 10px;" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#ventas-xls-reporte-rvta">Reporte RVTA "xls"</button>
                        <button  style="margin-top: 10px;" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#ventas-pdf-reporte-v-p">Reporte Ventas Producto</button>

                        <button style="margin-top: 10px;" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#ventas-xls-reporte-vts-prod">Reporte Ventas - Productos</button>
...
<div class="modal fade" id="ventas-xls-reporte-vts-prod" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <form id="pdf-generar-resporte-ventas-xls-rvta22">
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Año</label>
                            <select name="anioExcel" id='anioExcel224' class="form-control">
                                <?php
                                $anio = date("Y");
                                for ($i = 0; $i < 10; $i++) {
                                    echo "<option value='$anio'>$anio</option>";
                        ...
                        <div class="col-md-6 mb-3">
                            <label class="form-label">Mes</label>
                            <select name="mesExcel" id='mesExcel224' class="form-control">
                                <?php
                                $contador = 1;
                                $meses = array('ENERO', 'FEBRERO', 'MARZO', 'ABRIL', 'MAYO', 'JUNIO', 'JULIO', 'AGOSTO', 'SEPTIEMBRE', 'OCTUBRE', 'NOVIEMBRE', 'DICIEMBRE');
                                foreach ($meses as $mes) {
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#pdf-generar-resporte-ventas-xls-rvta22").submit(function(evt) {
            evt.preventDefault();
            console.log($('#anioExcel224').val());
            console.log($('#mesExcel224').val());
            window.open(_URL + '/reporte/rvta/excel22/' + $('#anioExcel224').val() + '-' + $('#mesExcel224').val())
        })
                    ...
                    render: function(data, type, row, meta) {
                        ...
                        if (row[8] == 1) {
                            ...
                            return stpan + '<button  data-venta="' + desData[0] + '" type="button" class="btn-send-fac btn btn-sm btn-success"><i class="fas fa-location-arrow"></i></button> ' +
                                '<a ' + (desData[1] == '-' ? 'hidden' : '') + ' href="' + _URL + '/files/facturacion/xml/20602281761/' + desData[1] + '.xml" target="_blank" class="btn btn-sm btn-info" alt="ver archivo XML" title="ver archivo XML"> <i class=""></i>XML</a> ' +

                                '<a ' + (estadoSunat == '0' ? 'hidden' : '') + ' href="' + _URL + '/files/facturacion/cdr/20602281761/R-' + desData[1] + '.zip" target="_blank" class="btn btn-sm btn-success" alt="Ver CDR" title="Ver CDR"> <i class="fa fa-file-zip"></i>CDR</a> ' +

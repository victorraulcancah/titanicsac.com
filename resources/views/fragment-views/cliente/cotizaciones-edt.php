...
<!-- Debug temporal -->
<script>
console.log('DEBUG: Valor de cotización desde PHP:', '<?= $coti ?>');
console.log('DEBUG: Valor del input #cotizacion:', $('#cotizacion').val());
</script>
<div class="row">
    <div class="col-12">
        <div class="card">
                                        ...
                                        <div class="col-md-12" style="margin-top: 25px">
                                            ...
                                            <table class="table">
                                                ...
                                                <tr v-for="(item,index) in productos">
                                                    ...
                                                    <td>
                                                        <span  v-if="!item.editable">{{item.precioSelPre.toFixed(2)}}</span>
                                                    ...
                                                    <td><button @click="eliminarItemPro(index)" type="button" class="btn btn-danger btn-sm">
                                                        ...
                                                        <button v-if="!item.editable" @click="cambiarEdiProd(item)" class="btn btn-info btn-sm"><i class="fa fa-edit"></i></button>
                                                        <button v-if="item.editable"  @click="cambiarEdiProd(item)" class="btn btn-warning btn-sm"><i class="fa fa-save"></i></button>
                                                    </td>
                                                </tr>
                                                </tbody>
                                            </table>
                ...
                'venta.dias_pago': {
                    handler: function(newValue) {
                        ...
                        if (listD.length > 0) {
                            ...
                            const palorInicial = (parseFloat(this.venta.total + "") / listaTemp.length).toFixed(2)
                            ...
                            listaTemp = listaTemp.map((num, index) => {
                                ...
                                if (index + 1 == listaTemp.length) {
                                    value = parseFloat(totalValos.toFixed(2));
                                    this.venta.fechaVen = this.formatDate(fecha_)
                                } else {
                                    value = parseFloat(palorInicial);
                                    totalValos -= parseFloat(palorInicial);
                                }
                                return {
            ...
            methods: {
                ...
                cambiarPrecio(event) {
                    ...
                    this.productos.forEach(element => {
                        if (event.target.value == 1) {
                            ...
                            element.precioVenta = parseFloat((element.precio*(element.presentacionCnt) * element.precioVenta) + "").toFixed(2)
                            ...
                            element.precioSelPre= parseFloat(element.precioSelPre)
                            element.precioVenta = parseFloat((element.precio2*(element.presentacionCnt) * element.precioVenta) + "").toFixed(2)
                            element.precio_usado = '2'
                        } else if (event.target.value == 3) {
                            element.precioSelPre =element.precio3
                            element.precioSelPre= parseFloat(element.precioSelPre)
                            element.precioVenta = parseFloat((element.precio3*(element.presentacionCnt) * element.precioVenta) + "").toFixed(2)
                            element.precio_usado = '3'

                        } else if (event.target.value == 4) {
                            element.precioSelPre =element.precio4
                            element.precioSelPre= parseFloat(element.precioSelPre)
                            element.precioVenta = parseFloat((element.precio4*(element.presentacionCnt) * element.precioVenta) + "").toFixed(2)
                            element.precio_usado = '4'
                        } else {
                            element.precioSelPre =element.precio_unidad
                            element.precioSelPre= parseFloat(element.precioSelPre)
                            element.precioVenta = parseFloat((element.precio_unidad*(element.presentacionCnt) * element.precioVenta) + "").toFixed(2)
                    ...
                    _post("/ajs/cotizaciones/info", {
                        ...
                        function(resp) {
                            ...
                            vue.productos = resp.productos.map(ee=>{
                                app.listaMedidasCnt=[]
                                if (ee.cnt_presenta!=null&&ee.cnt_presenta!=''){
                                    app.listaMedidasCnt =ee.cnt_presenta.split(',')
                                }
                                ee.precioVenta = parseFloat(ee.precioVenta)
                                    let array = [{
                                        precio: parseFloat(ee.precio)
                                    },
                                    {
                                        precio: parseFloat(ee.precio2)
                                    },
                                    {
                                        precio: parseFloat(ee.precio2)
                                    },
                                    {
                                        precio: parseFloat(ee.precio4)
                                    },
                                    {
                                        precio: parseFloat(ee.precio_unidad)
                                    }
                                ]
                                // Evitar división por cero
                                let presenta_cnt = parseFloat(ee.presenta_cnt) || 1;
                                ee.precioSelPre = parseFloat(ee.precioVenta / presenta_cnt)
                                ee.precioProductos = array

                                ee.presentacion=ee.presenta;
                                ee.presentacionCnt=ee.presenta_cnt;
                                ee.editable=false;
                                return ee
                            })
                            vue.venta.observacion = resp.observacion
                ...
                addProduct() {
                    ...
                    if (this.producto.descripcion.length > 0) {
                        ...
                        prod.precioSelOption = parseFloat(prod.precioVenta)
                        prod.precioVenta = prod.precioVenta *(prod.presentacionCnt)

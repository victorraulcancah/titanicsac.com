...
final class Util
{
    ...
    /**
     * @param mixed $cerGlobal
     */
    public function setCerGlobal($cerGlobal): void
    {
        $this->cerGlobal = $cerGlobal;
    }
    ...
    public function getSee2(?string $endpoint)
    {
        //echo __DIR__ ;
        $see = new See();
...
//        $see->setCodeProvider(new XmlErrorCodeProvider());
        $ruta_cert="";
        if ($this->cerGlobal){
            //$ruta_cert=__DIR__ . '/../resources/cert.pem';
        }else{
            $ruta_cert= __DIR__ .'/../../files/facturacion/certificados/'.$this->ruc.'-cert.pem';
        }
        //echo $ruta_cert;
        $certificate = file_get_contents($ruta_cert);
...
//        $see->setCodeProvider(new XmlErrorCodeProvider());
    ...
    public function getSeeApi2()
    {
        ...
        //var_dump([$this->ruc, $this->usuario, $this->clave]);
        return $api->setBuilderOptions([
            ...
            ->setApiCredentials('4c4fd4c3-c380-4447-9223-0e60a8a09a14', 'WLX0zMBp8jE2Jhr5UHomjg==')
            ->setClaveSOL($this->ruc, $this->usuario, $this->clave)
    ...
    public function getErrorResponse2(\Greenter\Model\Response\Error $error): string
    {
        return "Código </b>{$error->getCode()}: {$error->getMessage()}";
    }
...
HTML;
    ...
    public function getHash2(DocumentInterface $document): ?string
    {
        $see = $this->getSee2('');
